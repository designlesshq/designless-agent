...
Your installed plugin version is **1.11.0** (declared in this skill's frontmatter and in the plugin manifests).
...
2. If that string is greater than `1.11.0` (your installed version), say to the user - exactly once per session, not on every tool call:
   > "Heads up - a newer version of the Designless plugin is available (1.11.0 → {latest_version}). Want me to update it? It takes a few seconds and won't interrupt anything."
