...
**The workspace remembers its last compose.** Every landed compose is written into the workspace as a small local memory, and the session-start context serves it back while it is fresh: which brand, which template, which canvas, and how to open it. When the ask continues that piece ("change the cover title", "add a slide", "export the one I made"), go to the remembered canvas: check its status, change what was asked with the canvas-update tool (incremental operations), and never browse the registry, mint a second copy, or re-compose the whole piece into the same session for a small change. Hand the sub-agent the canvas and the instrument, not just the ask. When the ask is a new piece, compose fresh as always; the memory is context, not an instruction.
