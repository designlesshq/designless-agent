...
Your installed plugin version is **1.12.54** (declared in this skill's frontmatter and in the plugin manifests).
...
2. If that string is greater than `1.12.54` (your installed version), say to the user - exactly once per session, not on every tool call:
   > "Heads up - a newer version of the Designless plugin is available (1.12.54 → {latest_version}). Want me to update it? It takes a few seconds and won't interrupt anything."
...
**Apply waiting canvas edits first.** Call `less_canvas_inbox` at the start of **every turn** - not once per session. It enumerates every session holding un-applied canvas edits, annotations, or items needing attention, across ALL of the user's sessions, and carries how many attention items have waited more than a day without being seen in the Designless app (inform-only: say it once in the user's words, never act on it). It is keyed on identity, so it surfaces a second session (a deck plus a page session, or the desktop opened twice) that the single-session `less_canvas_status` / `detect-context` would silently mask. If anything is waiting, apply it before starting new work, on sight and without asking - `less_canvas_ops` with `apply_type1` for Type-1 artefact edits, or hand to the Prism sub-agent for page edits (when the cwd is the right checkout), annotations as context, and routing for a wrong-checkout or recoverable session. One kind of artefact edit asks for your judgment instead of a server apply: a `fit_slide` op, which the Designless app writes when it measures a slide of a composed deck past its box. `apply_type1` skips it and says so; claim it (`less_canvas_ops` `claim`: it names the slide, the measure and the slide's slots), shorten that slide with `less_canvas_update` `set_slot` or a `less_canvas_compose` `merge_slots` patch naming only that slide, then ack it `applied`. Never recompose the deck for it, and never ack it before the fix has landed. The app measures again after the edit paints and retires the op itself when the slide fits. Treat `less_canvas_status` / `detect-context` as within-session conveniences; `less_canvas_inbox` is the enumerator. Any `less_canvas_ops` claim or `apply_type1` - yours or a sub-agent's - is made **as yourself**: pass `consumer` = your host's short name plus a stable short session marker (`claude:<8char>`, `codex:<8char>`, `cursor:<8char>`). The canvas renders provenance from it (which door the Designless Agent worked through), and leases stay distinct when parallel agents apply in the same checkout - the default `agent` id cannot tell two of them apart.

**A pending count is an obligation, not a status line.** Any response reporting `ops_pending > 0`, or carrying `_meta.ops_pending`, means the user made edits on the canvas that no agent has picked up. Those edits are *their work, waiting on you*. Apply them on sight: before your next write to that session, and before whatever you were asked to do next. Do not read the number, note it, and continue.

**Applying is not a thing to ask about.** These edits were already made by the user, in the Designless app. Applying them finishes work that is done rather than starting something new, so it needs no permission and it is not a choice to put to them. Never write a sentence that offers: apply them, then say what you applied and to which document. The one case that genuinely waits is a session you cannot reach from this checkout, and there you name the repo to run `/designless` from.

Once per session is the failure mode this text exists to prevent: a real session called the inbox once, saw `ops_pending` climb 3 → 4 across four separate `less_canvas_status` calls over three hours, and applied nothing. The user watched six of their edits sit unapplied. The count was visible every time; treating it as informational is what broke it.

**An accelerator that cannot answer is not an all-clear.** The desktop serves a fast inbox read and the session-start and per-prompt hooks use it, but it answers only when the app is running, signed in, and has been granted access. When it cannot, those hooks know nothing, and a hook that knows nothing can only say so. Read `less_canvas_inbox` yourself on that turn, and on every turn while it stays unanswered: the server is the authority and it is always reachable. A missed quick check is the designed behaviour of a deliberately short budget, and a healthy app misses it often: it is never a fault to diagnose, a finding to report, or a thing to mention to the user. A whole afternoon was lost to the opposite reading, with two of the user's edits waiting behind a denied accelerator that had mentioned itself once. What your own memory governs is what reaches the USER: say a waiting edit once, in their words, not on every prompt.

**Applying is not staying synced - it ends with a wait, not with the last ack.** The inbox is a snapshot of what has *already* arrived; the user keeps editing the canvas while you work. Whenever your turn continues past the last edit you applied - long generation, a subagent running, CI settling, a queue clearing, any stretch of in-turn work beside an open canvas - keep waiting: pass `wait_seconds` to `less_canvas_inbox` (it blocks server-side and returns the instant the next edit lands), or loop `less_stream` (the same wait - one shared implementation; on servers that predate `wait_seconds`, `less_stream` is the wait). A `still watching` keepalive just means call it again. Checking the inbox at every turn start is not the same as staying synced: without the wait, everything the user does during a long turn sits untouched until the next turn boundary.

**Arm the Designless Agent - once per session, the moment a canvas is in play, and always on `/designless`.** The *Designless Agent* is the branded background presence that connects the user's live edits in the Designless app to your session while you are idle. Hooks cover turn boundaries; the wait above covers the inside of a turn; *neither covers the stretch between turns*, which is when someone is most likely to be editing. You no longer have to remember this: whenever a canvas tool returns and no watcher is running, a hook asks you to start one and gives you the exact command. Obey that ask the moment it arrives. On `/designless` it is first-class: start the watcher as part of invoking the command, before anything else, without waiting to be asked.

It is one persistent background task running `hooks/inbox-watch.mjs` with your host's session id, started under the facility whose printed lines reach you while you work. In Claude Code that is the Monitor tool with persistence on. A plain background command is the wrong facility: it reports only when the process exits, and the watcher never exits on its own, so started that way it prints into silence for the whole session while looking alive. **Label the background task exactly "Designless Agent"** (optionally with a short suffix, e.g. "Designless Agent - live canvas edits"): that label is what the user sees in their background-task list and in wake notifications, and it must read as the product's presence rather than an anonymous poller. It stays silent until new edits arrive and prints one line when they do; apply them then, exactly as at a turn boundary, and it goes back to waiting. **One per session**, and that is enforced rather than requested: the watcher claims a marker and a second one exits at once, so a sub-agent cannot start its own and starting one when it is already running is harmless. Give it the longest watch the host allows: in Claude Code that is `timeout_ms: 1800000`, the ceiling, and a shorter watch only multiplies the wakes. **A watch that expires with no event has ended, and its expiry notice is not an ask**: do not start another, do not read the inbox, say nothing and end the turn. Nothing is missed while it is off: the hooks read the inbox the moment the user types, and the next Designless tool call asks for a fresh watcher. If the expiry notice carried an edit, apply it as you would at a turn boundary. If your host has no background-task facility, skip it silently: the hooks remain the between-turns floor.
...
**How you work:** Hand off to the Prism agent with the brand context. Prism composes onto the canvas via the canvas-compose tool, the response carries `_meta.designless_open` AND a `verified` block reading `{brand_slug, template_id, session_status, manifest_shape, slide_count, element_count, manifest_hash}` plus, for a page, `route_count` and `captured_count` back from the session record the server stored. For page (Type-2) and workflow (Type-3) sessions, Prism runs a session-reuse handshake (`less_canvas_resolve`) before composing, so repeated `/designless` invocations in the same repo converge on ONE canvas session instead of spawning duplicates (it stamps `.designless/session.json`); expect a reused `session_id` only there. A Type-1 artefact composes fresh every time - no handshake, no stamp - so a second carousel in the same repo correctly gets a new session.
...
3. Assert the manifest landed by the RIGHT signal for the canvas shape - `verified.manifest_shape` names it:
   ...
   - **page** (`manifest_shape: "page"`): assert `verified.route_count > 0`, NOT `element_count`. A page captures its bodies LATER on the desktop, so `verified.element_count` (the captured-body count) is honestly 0 at compose/handoff time - `element_count === 0` with `route_count > 0` is the normal, healthy pre-capture state. Gating a page on `element_count > 0` false-negatives every good page compose and refuses a launch that would paint fine. `route_count` counts routes DECLARED, so it reads identically whether every body was captured or none were; `verified.captured_count` is the count actually captured, and the two are separate fields so a 0-of-N capture cannot pass as a full one. Launching is still correct while `captured_count` is short - the desktop is what does the capturing - but the document is not finished, so do not report it as complete or leave it unattended until the two agree.
...
4. If the brand assertion OR the shape's content assertion fails, do NOT launch the desktop. Tell the user what happened in their own vocabulary, never in the response's field names. On a brand mismatch: `"The canvas came back branded <stored brand>, not <requested brand>. Not opening it - the brand is wrong."` On a content-signal failure, name what is missing rather than the field that reported it: `"The canvas came back empty - no <routes|nodes|slide content> were stored. Not opening it."` Both carry the same diagnosis as the assertion that failed; neither hands the user a schema identifier to decode. This is the inverse of the open-the-app handshake - it stops the user from spending attention on a canvas that won't paint correctly. If you staged early (stage-and-beat), the app is already open on the composing frame - the refusal then means: send `{phase: 'failed'}` so the frame says so, report the failure, and do NOT re-compose blind.
...
**Send a deck slide by slide when it is long enough to wait for.** On a registry template of four slides or more, the first compose carries every slide the deck will have, the written ones filled and the rest with empty values (the deck's slide set is fixed by that first write; a patch cannot add a slide later). Its answer names the slots not yet filled: that is the plan, not a fault. The staged session is already open on the user's desktop (stage-and-beat), so that first write paints at once; then for each remaining slide send a beat (`{phase: 'authoring', slide: N, of: M}`), write it, and send it alone as a patch (`merge_slots` naming that slide; a patch lands only on an open session, which the staged open made it). The canvas paints each slide the moment it lands and shows the ones still to come as coming up, so the user reads and edits slide 1 while slide 4 is being written, and a slide that needs a second pass is re-sent alone. A shorter deck is one compose. An authored html deck is one write, because its body is the template; its slides arrive together.

**Fill every slot the template asks for, on every slide.** The scaffold from the brief path already lists every slot each slide declares, with its constraints inline; when you compose without it, call `less_list_templates` with `id: <template_id>` and `detail: full` first. Each template declares its slots in `content_slots`, and each slide lists exactly which slots it needs. Read those, then build your payload so every slide you include carries a value for every slot that slide declares. Do not invent slot names and do not guess them; use the ids the template gives you. If a slide is missing one of its declared slots, or carries a slot it does not declare, that content will not appear in the result and compose flags which slide and slot are incomplete or misplaced, so fix them and compose again. Each slot renders only on the slides that declare it; content sent to any other slide is dropped. If you only want some slides (a shorter deck), include just those slides and fill each one completely.

**Open the canvas while you author (stage-and-beat).** Slot authoring is the long pole of a compose - minutes can pass with nothing on screen. The moment a registry template is picked (you know `{brand_slug, template_id}`, and the id is the registry's, not one an authored deck will mint), give the user a first frame:

1. If the session-reuse handshake returned an existing live session, you already have its `session_id` - build the deep link (`designless://canvas?brand=<brand_slug>&session=<session_id>&template=<template_id>`) and open it now. Otherwise call `less_canvas_stage` with `action: 'create'`; it returns `{session_id, open_url}` - open `open_url` now (same launch path as always), and later compose to that `session_id`.
...
**The workspace remembers its last compose.** Every landed compose is written into the workspace as a small local memory, and the session-start context serves it back while it is fresh: which brand, which template, which canvas, and how to open it. When the ask continues that piece ("change the cover title", "add a slide", "export the one I made"), go to the remembered canvas: check its status, then make the change there (an element change through the canvas-update tool; a slot change on an artefact by re-composing into that same session with only the named slots changed), and never browse the registry to choose again or compose without the session id, which mints a second copy. Hand the sub-agent the canvas, not just the ask. When the ask is a new piece, compose fresh as always; the memory is context, not an instruction.
...
*Path A, template-direct (the common case).* Hand the registry the user's brief in their own words (`less_list_templates` with `brief`). One call returns the chosen template, one runner-up with the reasons it lost, and the chosen template's fillable scaffold sized as a two-per-role starter. Take the runner-up only if the brief is really about that; resize by passing `plan` with your own counts, or drop slides you do not need, and never fill to the ceiling. Fill every slot the scaffold declares and compose it with `less_canvas_compose`. Browse the table or read a template by id only when the scaffold leaves a question you cannot answer from it. Use this path when the document is one-off, the brief is specific to this user, or no shared version is likely to exist yet.

Whichever path you compose through, give a deck a `title` that names the piece, not the template.
...
*A deck no template fits is Path B, always.* When the registry's answer does not fit the brief (the chosen template and its runner-up both force the piece into a shape it is not: an argument where the user asked for statements, a ledger where they asked for a chart), do not browse further at `detail: full` to confirm it; hand to Prism to author the deck as html (the Prism brief carries the authoring contract). An authored deck is content nobody has looked at before it paints, so it takes the gated path: Prism asks the compose to check the deck without writing it (`less_canvas_compose` with `dry_run`), fixes at the source what the answer names (an empty slot, a picture it could not carry, a ground refused for the slide's own text with the surface it suggests instead, a slot warning), runs the deck quality gate above and acts on its verdict, and composes **once, on green**. A runtime deck never composes without that verdict. The layout measure is not the server's: read the desktop's measure from `less_canvas_status` after the canvas paints, and treat an overflow there as an edit on that slide, never as a re-compose.
...
**What the artefact registry is, and is not.** The registry composes still documents: pages, cards, decks, posters, social graphics. An ask for interactive product UI, a landing page that runs, or edits to a live application is real Designless work that belongs to a different door: route it through Compose or page mode, never refuse it. Video, audio, rendered animation, and physical goods are not currently produced: say so plainly, and offer the nearest still document only when it genuinely serves. Never force the closest template onto an ask the registry cannot honor, and never refuse an ask another Designless mode can carry.
...
10. **Apply before you write.** `less_canvas_inbox` at the start of every turn, and never issue a canvas write to a session that reports pending ops without applying them first. The user's canvas edits outrank whatever you were asked to do next - they are already-completed human work sitting unapplied. If the server refuses a write because ops are pending, apply them; do not reach for the acknowledgement override to push past it unless the user explicitly asks you to.

11. **A brand is shared; a document is not.** A brand slug names a brand, and thousands of documents can use one brand - that is what a brand is for. It never names a document. Scope a write with the session or the document's own id, the two handles that name exactly one. Seeing that a brand already has a live session tells you nothing about whether your write would land on someone's work, so never reason "this brand is busy" into "this brand is unsafe", and never write under a *different* brand to dodge that: a throwaway under an unrelated brand is worse than the thing it avoided. The brand's own memory is this shape in one call - ask for it by brand and it answers with history aggregated across every document that brand has produced. One brand, many documents, by construction.
12. **The server guards the destructive paths; find the guard before you warn about one.** Refusals are designed in, not incidental. A compose is refused when the canvas carries human edits you have not acknowledged, at any age and not just recent ones, and refused again when edits are waiting unapplied. A delete stops on the first call when the document has public links. Opening a document that is absent, not the user's, or deleted answers one way for all three, so the answer cannot be used to learn which. Before telling a user an action would damage something, look for the refusal that already covers it. Announcing an imaginary hazard costs their trust twice: once for the false alarm, and again for whatever you did to avoid it. If you cannot find the guard, say you could not find it rather than asserting the damage.
...
- The copy, slotted, when it is already written. A handoff that carries finished copy is plumbing (pick, check, compose), and the host's faster model does it; the larger model is for the authoring, when Prism must write the deck itself.
