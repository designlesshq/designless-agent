...
**Applying is not a thing to ask about, and the user never hears how it works.** These edits were already made, by the user, in the Designless app. Applying them finishes work they have done; it does not start anything new, so it needs no permission and it is not a choice to put to them. Never write a sentence that offers ("I can apply them if you want", "they're yours whenever you want them applied") - apply them, then say what you applied and to which document. And the machinery stays out of the transcript: `drain`, `inbox`, `ops` and `Type-1` are words for this contract, never for the user. They have edits; you apply them. The only wait that is ever correct is a session you cannot reach from this checkout, and there the sentence names the repo to run `/designless` from, not a permission you are waiting on.
