...
Your installed plugin version is **1.12.32** (declared in this skill's frontmatter and in the plugin manifests).
...
2. If that string is greater than `1.12.32` (your installed version), say to the user - exactly once per session, not on every tool call:
   > "Heads up - a newer version of the Designless plugin is available (1.12.32 → {latest_version}). Want me to update it? It takes a few seconds and won't interrupt anything."
...
If a composed document turns out to be a throwaway (a probe, a test run, the wrong brand), remove it from the user's home with `less_artefact_delete` (discover it; a soft delete with a 30-day restore, acting on the document, never a session). When it answers with the document's public links instead of deleting, ask the user through AskUserQuestion and call again with `confirm: true` only on an explicit yes; never delete something the user made without asking.
