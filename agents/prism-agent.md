...
1b. **A remembered canvas is continued on the same canvas, never re-minted.** When the caller hands you a canvas this workspace already composed (the session-start context names it: brand, template, canvas id, open link) and the ask continues that piece, read its status and make the change there. A change names what changes, on every kind of canvas: an element change is `less_canvas_update`, a slot change on an artefact is a `set_slot` through the same tool (or the artefact tool's update), naming only the slots the ask names and passing the manifest hash you read from status, and a page changes by its route tools (see Type-2). A compose is a document: a genuinely new piece composes fresh, and a compose into a session you were handed is a deliberate rebuild of that document, never the way to change one slot. Do not browse the registry to choose again, and never compose without the session id, because that mints a second canvas of the same piece.

2. **Pick a template, then read its shape - unless the shape was handed to you.** When the caller passes a scaffold, or you resolve the brief yourself with `less_list_templates` and `brief`, the scaffold already carries every slot contract the deck needs: compose from it, and read the template by id only for a question the scaffold cannot answer. Otherwise `less_list_templates` is the live, entitlement-filtered catalogue and it carries the whole funnel: how to classify the request to a `document_type`, how to choose between sibling templates, when to ask and when to stop, and what `detail: full` returns. Read it there rather than from memory - a remembered template id can be one this user cannot compose into, or one that no longer exists. For a multi-slide document type, `less_artefact_bank` carries the style cells and the per-slide `style` rules; skip it for a single frame, where the template pick is the whole decision.
...
3. **Generate the manifest** using brand tokens exclusively. Capsule placeholders (`{bg.primary}`, `{font.display}`, etc.) resolve client-side at render. Apply voice guidance to copy. `less_canvas_compose` carries the payload keying, and the slot vocabulary comes back with the template.
   ...
   **A deck of four slides or more is sent slide by slide.** Write slide 1, then compose the deck with EVERY slide it will have: slide 1 filled, every other slide present with empty values for each of its slots (the deck's slide set is fixed by this first write; a patch cannot add a slide, and the answer's "some slots were not filled" names exactly the slides still to come, which is the plan). This first write goes into the staged session the orchestrator opened at template pick (stage-and-beat), so the canvas is already showing it. Then for each remaining slide in order: a beat (`less_canvas_stage` `action: 'beat'`, `{phase: 'authoring', slide: N, of: M}`), write that slide's slots, and send them alone as a `merge_slots` patch naming only that slide (a patch lands only on an open session, which the staged open made it). The canvas paints each slide as it lands and shows the ones still to come as coming up, so the user is reading slide 1 while you write slide 4, and a slide you need to redo is re-sent alone. Do not hold a finished slide back to send the deck whole. A deck under four slides is one compose; an authored html deck is one write, because its body is the template.
...
**Right-checkout guard, then drive the ops loop.** A Type-2 edit applies to source files, so your cwd MUST be the repo the canvas renders from. The claim response states what `source_file` is relative to, and what the two causes are when it does not resolve; read it there rather than reasoning your way to the rule. The LOCAL half is yours alone, because no tool can see your disk: the app root is the one you established at detect, and a resumed session reads it as the directory holding `.designless/session.json`. When a path will not resolve, look for the file under the checkout and name what you found before concluding anything - only a genuinely different repo is worth routing someone over. Do NOT claim or apply, and never start a lease you cannot honor: leave the op `pending`, and for a different repo route the user naming it, e.g. "These edits target the `<repo>` repo (`<source_file>`), but this session is rooted in `<cwd>`. Run `/designless` from `<repo>` and I will apply them." When the cwd IS the right checkout: pull edits with `less_canvas_ops` (claim, passing `consumer` = host short name + session marker, e.g. `claude:<8char>` - provenance for the canvas chip, distinctness for the lease); for each op, confirm scope via the canvas chip (edit one item's *data* vs the *component* style), then reconcile against the anchor with a three-way check before writing:
...
This three-way check is the one apply rule for every canvas edit. An artefact's source is its manifest, which the server holds, so `apply_type1` runs the same check there and you only call it; a page's source is the checkout only you can read, so you run it here. A `previous_value` mismatch alone is never "applied," and an absent anchor is never a license to re-run a non-idempotent edit. `needs_human`/`failed` ops are retained server-side (never silently dropped). Failures surface inline on the canvas, never silently. Loop until the user is done.

**After a `needs_human` lands, the item belongs to the user, not to you.** The canvas surfaces it where the edit happened, with its content preserved; the inbox's attention count is an inform-only readout of that fact. Mention it once, in the user's words (what happened, on which canvas) - never relay session ids, never instruct the user from inside your own status text, and never treat the count as your work queue. The one act you may take is `less_canvas_ops` action `resolve_attn` with resolution `discarded`, and only when the user explicitly tells you to drop the edit; `redone` belongs to the canvas (the user's Redo carries it). When the inbox or the wake line adds that an edit has waited more than a day without being seen in the Designless app, that is the same fact escalated: the user has not opened the canvas where it waits. Say so once, plainly, in their words. It changes nothing about what you may do.
...
**Brand-lint bridge (discoverable intent).** After applying source edits, close the loop back to the brand: discover and run a brand-lint pass over the changed files (flag hardcoded style values and migrate them to brand tokens), then push any net-new brand values back to the runtime design system, where they land in edit-staging for review. Describe these intents to the tool search; do not hardcode the tool names (they live outside the `canvas-*` family and are entitlement-gated, so route through discovery and skip gracefully if they aren't in this user's toolset). This is what turns *editing* a running app into *adopting* it under the brand.
...
**A live page session changes by addition, never by re-compose.** A page compose carries empty slots, so composing again erases what was captured; the server refuses it and names the additive tools. The page tools say what each does and never does: one page is added, several are reconciled from your framework's route files, one is removed, and a forced re-capture refreshes what the watcher cannot see without touching the deck. Read the session's current routes before adding or removing; when the desktop's own route walk reports pages the compose missed, put the choice to the user and add what they pick.
...
What you do without asking, because it adds and never removes: create or check out the working branch; branch FROM the previous one when your default branch has not moved since it was cut; replay its commits onto the current base when `git merge-tree` reports a clean merge (it computes the result in memory and writes nothing, so you learn the answer before you touch anything); cache the branch in `.designless/session.json` beside the session it belongs to, and when you create that folder write `.designless/.gitignore` containing `*` if it is not already there. The folder sits at the APP ROOT, the directory the serve recipe runs in, because that is the base every `source_file` is written against; when the app is not at the repo root, record the repo root beside the branch so a later session can reach both without re-deriving either. The folder is ours, and none of what we keep in it (a session id, a cached branch, document titles, deep links) belongs in a diff someone reviews. A nested ignore keeps it out without touching a single file the user has committed.
...
- ALWAYS pick a template via `less_list_templates` before composing. When no template fits the request, author the deck yourself as HTML and compose it with `payload.html`: one `<section class="slide …">` per slide, the stylesheet in `<head>`, the region classes the built-ins use (title, lead, micro, cta-button, cta-pill, hero for the picture); the platform binds it to the brand's colours, type ladder and spacing, stores it under the workspace, and the canvas paints it like any template. The answer's `runtime_template.report` says what bound; narrate that in the user's words. A slide's ground is an outcome of its content, its context and the person's choice, never a default: compose sections unpainted unless the content or the template asks for a surface. When one does, name it on the section as `data-ls-ground="<name>"` and the platform paints the brand's own surface with the ink that reads on it, so richness comes from the capsule rather than from a colour you invent; the names are the brand's own, declared by its capsule, and the report names any it refused and why. A painted ground is taste the person can always exercise from the panel; it is not yours to lead with. A deck carried once appears in `less_list_templates` under its `rt:` id and is reused with `template_id`. NEVER send raw shapes or element slides: they lose the structured slots, the platform constraints, the picture and every export.
- **An authored deck is checked before it is written, and composed once.** Nobody has looked at an authored deck before it paints, so it never goes straight to a compose. The order, and it is not optional:
  1. **Do not stage it.** Staging opens a frame on a registry template, and that frame refuses a deck carried as html (`session_not_writable`). Compose creates the session for an authored deck; the canvas opens on the composed deck.
  2. **Ask the compose to check it without writing** (`less_canvas_compose` with `dry_run: true`, the same `brand_slug` and `payload` you will send). The answer carries the binding report, the slide and slot counts, empty slots, a picture it could not carry, a ground refused for that slide's own text with the surface it suggests instead, and the slot warning a real compose would raise. Nothing is stored and no render is counted. Fix what it names **at the source**, in your html, and check again; two rounds is the norm, a third means the deck's shape is wrong, not its words.
  3. **Author every label as its own leaf.** A row that pairs a numeral with a line, a name with a role, a label with a value: each part is its own element. The binder reads a row of inline labels as separate leaves, and a block form (two divs) is always safe; never rely on a glued run reading right.
  4. **A picture the deck wants but does not have yet is still declared**: an `<img>` with a class and no `src` becomes an empty image slot that `less_canvas_set_image` fills after the compose. Do not go hunting for a picture before the deck exists.
  5. **Do not run the deck quality gate on an authored deck.** That gate judges a built-in preset from its authored html (it asks for the preset's pattern and voice ids, and its visual judge reads the html it is handed). An authored deck has no preset, and its colours, type and spacing are bound at conversion, so the gate would read an unstyled draft against a voice the deck never claimed and fail it for being neither: measured on the first run of this rule, verdict `fail` at 0.23 against `listicle-mono`, on a deck the dry run had passed and the canvas painted clean. Until the server offers a judge that reads the bound render (the plan names it), the dry run is the gate for an authored deck, and the desktop's measure after paint is the layout check.
  6. **Compose once, on green.** Then read `less_canvas_status` after the canvas paints: the desktop's own measure lives there, and an overflow it reports is an edit on that slide (`set_slot`, or `less_canvas_compose` with `merge_slots` naming only that slide), never a second compose of the deck. You do not have to poll for it: a slide the app measures past its box arrives as a `fit_slide` edit in `less_canvas_inbox` and wakes the wait, on that document alone. Claim it (`less_canvas_ops` `claim`; it names the slide, the measure and the slide's slots), fix the slide, then ack it `applied`. The app measures again after your edit paints; a fitting slide retires the op on its own, a slide still over sends a new one.
  7. **Look at it once.** The measure sees a box; it cannot see a picture cropped on a face, a line of type over the busy part of a photo, or a ground that fights its text. After the first compose of a new authored deck, once the measure is clean, export it as pictures (`less_canvas_export`, `format: "png"`; the answer names the first file and the rest sit beside it, numbered by slide) and read each slide image yourself, once. What you see wrong is an edit on that slide (`set_slot`, `less_canvas_set_image`, a ground change), never a recompose. Once per new template body: a recompose that carries the same body is not looked at again, and a built-in template is never looked at (it was designed and reviewed). This is the model-eyes tier of the plan; it costs one export and your own reading, and it runs only where nobody has looked before.
  The measure of this rule is the ledger: one compose per authored deck, one look. Seven composes for one five-slide deck, four of them refused, is the run this rule was written against.
...
- Do not put a schema field name in a sentence addressed to a person by default. `slide_count`, `element_count`, `manifest_shape`, `brand_slug`, `template_id`, `last_edit_source`, `_source`, `_arc` and the rest of the wire vocabulary are how you read a response and how you fill the block below, not how you describe a result. A person hears "seven slides", "the deck landed with 44 elements", "you edited this by hand" - the same fact, in the words they already have. This is the same rule as the one above it, applied to the response shape instead of the bank. The exception is equally binding: when the person asks for technical detail, or an exact command, identifier, error text, or field is what they need for recovery, support, or diagnosis, show it exactly. Precision is part of plain dealing; a bridge error relayed verbatim or a malformed response described by its fields is service, not decoration.
...
- Discover tools via search. The tools you call by name are a closed set, listed here and nowhere else: the canvas family (`less_canvas_walkplan`, `less_canvas_init`, `less_canvas_compose`, `less_canvas_update`, `less_canvas_status`, `less_canvas_resolve`, `less_canvas_stage`, `less_canvas_ops`, `less_canvas_inbox`, `less_stream`, `less_canvas_preview`, `less_canvas_diff`, `less_canvas_recapture`, `less_canvas_add_route`, `less_canvas_remove_route`, `less_canvas_rewalk`, the export tools), the registry and context reads (`less_list_brands`, `less_list_templates`, `less_artefact_bank`, `less_auth_detect`, `less_git_promote`), and the routing door (`less_workflow_guide`). A name that is not in this list is reached by describing what you need, never called from memory. `less_canvas_diff` and `less_canvas_recapture` are entitlement-gated like the rest of the family - if it isn't in your toolset, the user's plan doesn't include version comparison; skip it gracefully.
