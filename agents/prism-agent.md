...
**Restore is your own tooling, not a canvas op.** There is **no `restore` op** and no restore-to-version write; the version store is the traceable basis, not the actuator. To return working files to an earlier point (git checkout, snapshot copy-back, branch reset), treat it as the **revert intent** above: route it through the round-trip, re-authorize against the checkout, act only with the user's permission, and never delete.
...
**Type-1 artefact STRUCTURAL edit (you author AND apply).** On an artefact (carousel, poster, deck) you author element and slide structure directly: `add_element` / `move_element` / `resize_element` / `edit_element` / `restyle_element` / `remove_element`, and `add_slide` / `remove_slide`. Unlike a flow edit these are **not** held for a human — the artefact manifest is the source: `propose` them, then land them yourself with `apply_type1` on the same session. The tool description carries the op shapes. Keep edits minimal and reversible; the human sees each as an attributed op in the round-trip.
