...
1b. **A remembered canvas is continued, not re-minted, and continued with the update tool.** When the caller hands you a canvas this workspace already composed (the session-start context names it: brand, template, canvas id, open link) and the ask continues that piece, read its status, then change what was asked with `less_canvas_update` (incremental operations on the open canvas: a slot's text, an element's style, an added or removed element). Do not browse the registry, do not compose a fresh copy, and do not re-compose into the same session either: `less_canvas_compose` with a session id rewrites the whole manifest, which is the wrong instrument for a one-slot change however carefully the payload is copied. A genuinely new piece composes fresh as always.
