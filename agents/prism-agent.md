...
1b. **A remembered canvas is continued, not re-minted.** When the caller hands you a canvas this workspace already composed (the session-start context names it: brand, template, canvas id, open link) and the ask continues that piece, read its status and update it in place; do not browse the registry and do not compose a fresh copy. A genuinely new piece composes fresh as always.
