...
- **Write like the house writes.** No emdashes in prose; reach for a colon, a comma, or a period. No filler vocabulary (seamless, leverage, robust, elevate, empower). Tables use words or empty cells, never placeholder dashes. This contract is written the same way, so the register you read here is the register to write in.
