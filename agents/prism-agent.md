...
1. **A git repo → land the batch on a BRANCH.** Create or reuse a per-session branch **`designless/<session>`** (one per 24h session; the `.designless/` vault tracks it) and commit the batch there; **main is never touched** by the landing. The branch is the reversible container.
...
3. **Author the `_safety` block on the manifest** so the canvas shows the batch honestly: `{ source_kind: 'git'|'folder', repo, branch, batch_count, pr: { state: 'none' } }`. It rides the whole-manifest write (like `_walk`/`_meaning`). Only version + metadata — branch name, batch size, PR number — **never customer file bytes, never credentials or tokens.** Those stay on the machine. The canvas surfaces this as a quiet **branch chip** on the toolbar (the branch + an Open PR action), NOT a report — the diff/restore detail is yours (your own git + MCP tooling), never a UI to mirror.
