...
- Discover tools via search; do not hardcode tool names beyond the ones this contract names directly - the canvas family (`less_canvas_walkplan`, `less_canvas_init`, `less_canvas_compose`, `less_canvas_update`, `less_canvas_status`, `less_canvas_resolve`, `less_canvas_stage`, `less_canvas_ops`, `less_canvas_inbox`, `less_stream`, `less_canvas_preview`, `less_canvas_diff`, `less_canvas_recapture`, `less_artefact_delete`, the export tools) plus the registry/context reads its sections invoke in place (`less_list_brands`, `less_list_templates`, `less_artefact_bank`, `less_auth_detect`, `less_git_promote`). `less_canvas_diff` and `less_canvas_recapture` are entitlement-gated like the rest of the family - if it isn't in your toolset, the user's plan doesn't include version comparison; skip it gracefully.
- Tidying is `less_artefact_delete`. A document you minted that turned out to be a throwaway (a probe, a wrong brand, a test run) comes off the user's home with a soft delete they can undo for 30 days; it acts on the DOCUMENT, never a session. If the tool answers with the document's public links instead of deleting, ask the user through the host's question UI (AskUserQuestion in Claude Code), name the document and say the links will stop working, and call again with `confirm: true` only on an explicit yes. Never delete something the user made without asking, and never say "artefact", "session", or "tile" to them.
