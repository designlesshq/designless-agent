...
- **An authored deck is checked before it is written, and composed once.** Nobody has looked at an authored deck before it paints, so it never goes straight to a compose. The order, and it is not optional:
  1. **Stage it first, with its document type.** The moment you know the deck's document type (before you write a word of it), `less_canvas_stage` with `action: 'create'`, `brand_slug` and `document_type`, no `template_id`: the canvas opens on a blank sheet of that type in the composing state, and the person watches the deck arrive instead of waiting on a closed window. Return the `open_url` to the orchestrator as always. Send `{phase: 'authoring', slide: N, of: M}` beats as you write each slide, `{phase: 'composing'}` before the compose, and `{phase: 'failed'}` if authoring dies. The compose that follows goes into THAT `session_id` and lands whether or not the app has opened it. (An earlier version of this rule said never to stage an authored deck because the compose into the staged session was refused; the refusal was the stage not yet opened, not the deck, and it is fixed.)
  2. **Ask the compose to check it without writing** (`less_canvas_compose` with `dry_run: true`, the same `brand_slug` and `payload` you will send). The answer carries the binding report, the slide and slot counts, empty slots, a picture it could not carry, a ground refused for that slide's own text with the surface it suggests instead, the slot warning a real compose would raise, and, with the Designless app open on this machine (it is, once you staged), the render laid out: a slide past its box, a picture box collapsed to a band, a label fused to its number. Nothing is stored and no render is counted. Fix what it names **at the source**, in your html, and check again; two rounds is the norm, a third means the deck's shape is wrong, not its words.
  ...
  6. **Compose once, on green.** Then read `less_canvas_status` after the canvas paints: the desktop's own measure lives there, and an overflow it reports is an edit on that slide (`set_slot`, an element op), never a second compose of the deck.
  The measure of this rule is the ledger: one compose per authored deck. Seven composes for one five-slide deck, four of them refused, is the run this rule was written against.
