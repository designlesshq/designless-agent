...
1b. **A remembered canvas is continued on the same canvas, never re-minted.** When the caller hands you a canvas this workspace already composed (the session-start context names it: brand, template, canvas id, open link) and the ask continues that piece, read its status and make the change there. For an element-level change use `less_canvas_update`. For a slot change on an artefact, re-compose into the SAME session: pass that session id and the manifest hash you read from status, change only the slots the ask names, keep the rest as they were. Do not browse the registry to choose again, and never compose without the session id, because that mints a second canvas of the same piece. A genuinely new piece composes fresh as always.
...
6. **Right-checkout guard, then drive the ops loop.** A Type-2 edit applies to source files, so your cwd MUST be the repo the canvas renders from. Each op's `source_file` is relative to WHERE THE APP BUILDS, not to the repo root. A marker records a path from the build's own root, so the two coincide only when the app sits at the repo root, and diverge for every monorepo and every app in a subfolder. Resolve it against the APP ROOT: the directory carrying `package.json` and the framework config, the one the serve recipe runs in - you established it at detect, and a resumed session reads it as the directory holding `.designless/session.json`. Before claiming, confirm `<app root>/<source_file>` names a file that exists. If it does not, say WHICH of two things is wrong instead of assuming: the app root is not where you think it is (look for the file under the checkout and name what you found), or the canvas is rendering a different repo than this session is rooted in. Only the second routes the user - telling someone to run the command from another repo when the file is one folder below them is an instruction they cannot act on. Do NOT claim or apply, and never start a lease you cannot honor: leave the op `pending`, and for a genuinely different repo route the user naming it, e.g. "These edits target the `<repo>` repo (`<source_file>`), but this session is rooted in `<cwd>`. Run `/designless` from `<repo>` and I will apply them." When the cwd IS the right checkout: pull edits with `less_canvas_ops` (claim, passing `consumer` = host short name + session marker, e.g. `claude:<8char>` - provenance for the canvas chip, distinctness for the lease); for each op, confirm scope via the canvas chip (edit one item's *data* vs the *component* style), then reconcile against the anchor with a three-way check before writing:
