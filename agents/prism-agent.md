...
5. **Compose the page session.** Call `less_canvas_compose` with a **page manifest** as the `payload`. You author this manifest the same way you author a Type-1 template manifest; the server persists it as-is (there are no separate `port`/`routes` params, they live inside the manifest) and the renderer fills `_source.slots` per captured route.
   ...
   **Hand off the moment compose returns — do NOT babysit the capture.** Capturing the routes is the CANVAS's job, not yours. Once `less_canvas_compose` returns its `verified` block and the `designless://…` open link, you are DONE with this turn: return the structure (below) so the orchestrator opens the canvas, and stop. Do NOT poll `less_canvas_status` (or anything else) waiting for the frames to finish — that is the anti-pattern that left a real run spinning for 20 minutes with no success/fail signal. The canvas captures each route progressively and OWNS the result: frames appear in the strip as they land, capture is bounded per route (a load/serialize timeout — it cannot run forever), a route that can't be captured paints an honest inline error in its own slide (a page-too-heavy, a 4xx/5xx, or an unreachable server — each with the right remedy) plus a per-frame **Re-capture**, and a reused or reopened session auto-retries the failed frames so it self-heals. Tell the user in product terms and let them watch the canvas, e.g. "Opening the canvas — it's capturing your <N> routes now; pages appear as they finish, and any that can't be captured will say why with a Re-capture button." The ONLY status read after compose is the defensive read on a later **follow-up** request (step 6 / "Defensive read"), never a wait-for-capture loop.
