...
**Applies to Type-2 page mode and Type-3 workflows ONLY — never Type-1 artefacts.** A page/workflow is the repo's ONE canvas (its running app, or its agentic structure), so repeated `/designless` invocations in the same repo must converge on ONE session, not spawn a duplicate each time. A Type-1 artefact (carousel, poster, tweet card, …) is the opposite — an independent one-off deliverable — so it is **never** deduped by repo: each artefact compose creates its own session, and one brand carries unlimited independent artefact sessions (see the skip note at the end). Run this reuse-first handshake **before composing** (step 8) for a page or workflow:
...
3. **Always pass `repo_remote`/`repo_head` to `less_canvas_compose`** (page and workflow) so a freshly created session carries the checkout identity a later `repo_match` needs.
...
Stamp shape: `{ session_id, bind_token, repo_remote, repo_head, stamped_at }`. Add `.designless/` to `.gitignore`. `bind_token` is a per-session secret that authorizes reuse — never commit it or log it elsewhere. **Skip this handshake entirely for EVERY Type-1 artefact** — compose fresh with no `session_id`, and do NOT read or write `.designless/session.json`, even inside a git repo (the stamp belongs to the repo's page/workflow session, not to a one-off artefact — reusing it would land your carousel on top of the user's app session). Do NOT pass `repo_remote`/`repo_head` on an artefact compose either: those are the repo-dedup identity and an artefact must stay independent (the server drops them for a non-page compose regardless, but don't send them). To iterate the SAME artefact within a conversation, pass the `session_id` the compose response already gave you (explicit continuation). This is what lets one brand hold hundreds of independent artefact sessions.
...
4. **Verify the markers wired in** (three-way diagnostic; framework apps only — see step 3): the dependency installed, the config was edited (the `wire` import/wrapper the tool named is present), and a dev build doesn't error. The annotator fails loud and never crashes the dev server - if it didn't wire (a version gate or loud no-op), surface the diagnostic and fall back. Do not compose a FRAMEWORK page session against unmarked source. A `static-serve` app is never marked and never needs to be — it has no build to mark.
...
1. **A git repo → branch FIRST, then land the batch on it.** The **server declares** the containment branch name and writes it to `manifest._safety.branch` at compose (for a page/workflow source-editing session): the exact string **`designless/<session>`** (the first 8 chars of the session id). **Use that exact name** — read it back from the session (`less_canvas_status` / the compose `verified` block carry the manifest) and `git checkout -b designless/<session>` **before your first edit**; reuse it on later edits in the same session. Commit the batch there; **main is never touched** by the landing. The branch is the reversible container. This is now **server-enforced**, not just convention (see the branch-gate note below).
...
3. **The `_safety` block on the manifest** shows the batch honestly: `{ source_kind: 'git'|'folder', repo, branch, pr: { state: 'none' } }`. For a git source-editing session the **server stamps it at compose** — you do not invent the branch; you check out the name the server declared. You still re-author `_safety.pr` after a consented Open PR (below). It rides the whole-manifest write (like `_walk`/`_meaning`). Only version + metadata — branch name, PR number — **never customer file bytes, never credentials or tokens.** Those stay on the machine. The canvas surfaces this as a quiet **branch chip** on the toolbar (the branch + an Open PR action), NOT a report — the diff/restore detail is yours (your own git + MCP tooling), never a UI to mirror.

**The branch is server-enforced — pass your current branch on every source claim/ack.** When you call `less_canvas_ops` action `claim` (draining source edits) or `ack` (reporting one applied), pass **`repo_branch`** = the checkout's current branch (`git rev-parse --abbrev-ref HEAD`). The server withholds source-mutating ops from any claim whose `repo_branch` is absent, `main`/`master`, or ≠ the declared branch, and returns **`on_branch_required`** with `required_branch`. When you see it: **`git checkout <required_branch>`** (create it if it doesn't exist), then re-claim — the withheld ops are still pending, nothing was lost, and main was never touched. On `ack`, a source op reported from the wrong branch is forced to `needs_human`, not recorded applied. Non-source ops (annotations, artefact Type-1) are never gated. This is standard git — no engine internals, just branch-before-you-edit made a hard rule.

**Never push, and never open a PR on your own.** Opening a pull request is an **invited, permission-gated** act the user takes first. The branch chip's one action is **Open PR**; when the user takes it, an `open_pr` intent arrives (drain it with `less_canvas_ops`). **Only then** run `gh pr create` under the **user's own** `gh` auth, and ack with the **real** PR url + number — re-author `_safety.pr = { state: 'open', url, number }`. The honest link appears only because the PR now exists; never show a link, number, or "opened" claim before consent (the client's optimistic stamp caps at `state: 'held'`, never `'open'`). Consent is **per-batch**, never remembered. If `gh` isn't authed or the push can't proceed, say so plainly and leave the branch as the safe container.

**Restore is your own tooling, not a canvas op.** Returning the working files to an earlier point (a git checkout, a folder snapshot copy-back, a branch reset) is the **revert intent** above — routed through the round-trip, re-authorized against the checkout, taken only with the user's permission, and never a delete. There is **no `restore` op** and no restore-to-version write; the version store is the traceable basis, not the actuator.
...
**Type-1 artefact STRUCTURAL edit (you author AND apply).** On an artefact (carousel, poster, deck) you can author element and slide structure directly: `add_element` / `move_element` / `resize_element` / `edit_element` / `restyle_element` / `remove_element`, and `add_slide` / `remove_slide`. Common shapes: *align or distribute* a set of elements = a batch of `move_element` (each carries the element's absolute target position); *"add a closing slide"* = `add_slide` with the new slide object. Each op names its target slide by `section_index` (the 1-based slide identity) **and** `slide_id` (the target slide's stable id — pass both). These need **no provenance and no node_id**, and — unlike a flow edit — they are **not** held for a human: the artefact manifest is the source, so you `propose` them, then land them yourself with `apply_type1` on the same session. Keep edits minimal and reversible; the human sees each as an attributed op in the round-trip.

**Page sessions take the six ELEMENT ops too.** On a page session (a live app on the canvas) `add_element` / `move_element` / `resize_element` / `edit_element` / `restyle_element` / `remove_element` are first-class. An element added to a page belongs to the **canvas**, never to the page itself: the user's code is not modified by these ops, and the page renders exactly as it did — use them for callouts, notes, badges, or any addition that annotates the page rather than changes it. `add_slide` / `remove_slide` stay artefact-only (a page's slides are its routes; add or remove pages with `less_canvas_add_route` / `less_canvas_remove_route` instead). Same flow as artefact struct ops: `propose`, then `apply_type1` — no human hold.

The rest of this section is the OTHER class — a **flow edit** on a captured Type-2 page, which changes the app's **shape** (the order of a route's runtime states, a bulk text change across many frames, a transition/guard, or add/drop a state). These are structural intents about the walk itself, held to a stricter contract because getting them wrong reshapes the user's app.
